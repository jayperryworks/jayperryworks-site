 ...
 * @returns {string}
 */
export function setBaseHue(options = {}) {
	const {
		name,
		value,
	} = options;

	if (value) return `--base-h: ${value}deg;`;
	return `--base-h: ${presetHues[name.toLowerCase()]}deg;`;
}
 ...
 */
export function setThemeStyle(theme = {}, inherit = false) {
	const {
		name,
		hue,
		custom,
	...
	if (name) styles.push(setBaseHue({ name }));

	// if a custom hue is passed as a prop, assign it to the base-h variable
	if (hue) styles.push(setBaseHue({value: hue }));
	...
	if (styles.length === 0 && !inherit) styles.push(setBaseHue({value: DEFAULT_HUE}));
